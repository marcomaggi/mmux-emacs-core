...
(cl-macrolet
    ((mmec--def (TYPESTEM SIGNED)
		(let* ((TESTNAME	(mmec-sformat "mmec-test-%s-number-type-is-signed" TYPESTEM))
		       (NUMTYPE		(mmec-sformat "mmec-%s" TYPESTEM))
		       (CONSTRUCTOR	NUMTYPE))
		  (if SIGNED
		      `(ert-deftest ,TESTNAME ()
			 (should (mmec-number-type-is-signed-p ,TYPESTEM))
			 (should (mmec-number-type-is-signed-p ,NUMTYPE))
			 (should (mmec-symbol-number-type-is-signed-p (quote ,TYPESTEM)))
			 (should (mmec-symbol-number-type-is-signed-p (quote ,NUMTYPE))))
		    `(ert-deftest ,TESTNAME ()
		       (should (not (mmec-number-type-is-signed-p ,TYPESTEM)))
		       (should (not (mmec-number-type-is-signed-p ,NUMTYPE)))
		       (should (not (mmec-symbol-number-type-is-signed-p (quote ,TYPESTEM))))
		       (should (not (mmec-symbol-number-type-is-signed-p (quote ,NUMTYPE))))))
		  )))
  (mmec--def char	t)
  (mmec--def schar	t)
  (mmec--def uchar	nil)
